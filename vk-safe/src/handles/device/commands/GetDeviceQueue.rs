...
pub trait GetDeviceQueue: DispatchableHandle<RawHandle = vk::Device> + DeviceAttributes {
    ...
    ///
    /// The returned Queues
    fn get_device_queues<'a, 't, Q: QueueFlags, X>(
        &'a self,
        family_config: &DeviceQueueCreateInfo<Self::QueueConfig>,
        ...
        (
            ...
            impl QueueIterator<'a, Self, Q, Tag<'t>> + use<'a, 't, Self, Q, X>,
        ),
        UnsupportedCapability,
    >
    where
        Self::Commands: vk::has_command::GetDeviceQueue<X>,
    {
        ...
        if family_flags.satisfies(capability) {
            ...
            let mut i = 0;
            let fptr = self.commands().GetDeviceQueue().get_fptr();
            let device_handle = self.raw_handle();
            ...
            let queue_iter = std::iter::from_fn(move || {
                if i == num_queues {
                    ...
                    unsafe {
                        ...
                        i += 1;
                        Some(make_queue(handle.assume_init(), self, &tag))
                    }
                }
            });
...
pub trait QueueIterator<'a, D, Q, T>:
    Iterator<Item: Queue<Device = D, Capability = Q, Family = T>>
{
}

impl<'a, D, Q, T, I> QueueIterator<'a, D, Q, T> for I where
    I: Iterator<Item: Queue<Device = D, Capability = Q, Family = T>>
{
}
